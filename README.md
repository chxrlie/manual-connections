...
To exclude LAN traffic from VPN routing (split tunneling), add `ROUTE_LAN_VIA_VPN=false`:

```
sudo VPN_PROTOCOL=wireguard DISABLE_IPV6=yes DIP_TOKEN=no AUTOCONNECT=true PIA_PF=false PIA_DNS=true ROUTE_LAN_VIA_VPN=false PIA_USER=p0123456 PIA_PASS=xxxxxxxx ./run_setup.sh
```
...
- [Prompt based connection](run_setup.sh): This script allows connections with a one-line call, or will prompt for any missing or invalid variables. Variables available for one-line calls include:
  ...
  - `DISABLE_IPV6` - yes/no
  - `ROUTE_LAN_VIA_VPN` - true/false; route LAN traffic via VPN (default: true). When set to false, common LAN subnets (192.168.0.0/16, 172.16.0.0/12, 10.0.0.0/8, 169.254.0.0/16) will be excluded from VPN routing.
